...
# 第一部分：加载包
#######################################################################
devtools::install_local("D:/downloads/CellChat-master.zip", dependencies = FALSE)
# 装好之后，再回头装 CellChat（这时候它会很快）
...
pericyte_clusters <- c(0, 4)  # 周细胞（PDGFRB+, CSPG4+, RGS5+, MYH11-）
vsmc_clusters     <- c(3)     # 平滑肌细胞（MYH11+, CNN1+, CSPG4低）

# 【额外建议】处理中间态 C2 和 未知群 C1
# 方法A：如果你只想保留 周细胞 和 平滑肌，可以将 C2 归为 VSMC，C1 作为杂质丢弃
 vsmc_clusters <- c(2, 3) 
# 方法B：如果你要细分，可以将 C2 单独命名为 "Intermediate_Mural"，C1 命名为 "Unknown"
# intermediate_clusters <- c(2)
# unknown_clusters      <- c(1)
...
cat("\n===== 开始细胞通讯分析 =====\n")
# 重新加载原始数据，保证包含所有细胞
scobj <- readRDS("scobj_annotated.rds")

# 提取 Mural cell 并聚类 (如果还没有聚类的话)
mural_obj <- subset(scobj, celltype == "Mural cell")
# 进行标准化和聚类...（同前）
# -----再次聚类 Mural cell，获取周细胞条形码 -----
mural_obj <- subset(scobj, celltype == "Mural cell")
mural_obj <- NormalizeData(mural_obj, verbose = FALSE)
mural_obj <- FindVariableFeatures(mural_obj, nfeatures = 2000)
mural_obj <- ScaleData(mural_obj)
mural_obj <- RunPCA(mural_obj, npcs = 30)
mural_obj <- FindNeighbors(mural_obj, dims = 1:10)
mural_obj <- FindClusters(mural_obj, resolution = 0.5)   # 你之前用的分辨率
mural_obj <- RunUMAP(mural_obj, dims = 1:10)

# 确认分群是否符合预期
DimPlot(mural_obj, label = TRUE)
# 获取 cluster 0,1,4 的细胞
pericyte_cells <- WhichCells(mural_obj, idents = c(0,1,4))
non_pericyte_mural_cells<-WhichCells(mural_obj, idents = c(2,3,5,6))
length(pericyte_cells)  # 看看数量

# ----- 2. 检查这些细胞是否全部在主对象中 -----
all(pericyte_cells %in% colnames(scobj))   # 现在应该会返回 TRUE
# 获得周细胞条形码
pericyte_cells <- WhichCells(mural_obj, idents = c(0,1,4))

# 现在检查应该全部在 scobj 中
all(pericyte_cells %in% colnames(scobj)) # 应该返回 TRUE

# 转为字符并替换
scobj$celltype <- as.character(scobj$celltype)
scobj$celltype[pericyte_cells] <- "Pericyte"
scobj$celltype[non_pericyte_mural_cells] <- "Non-pericyte mural cells"
...
# 添加 UROD_status 列
pbmc_pure$UROD_status <- ifelse(urod_exp$UROD > threshold, "UROD_WT", "UROD_KO")
...
VlnPlot(endo_sub, features = c("VEGFR1", "BSG", "ITGA6"), pt.size = 0) 
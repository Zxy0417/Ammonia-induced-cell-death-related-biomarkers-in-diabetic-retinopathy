...
Uni_glm_model<-function(x){
  ...
  Uni_glm_model<-data.frame('characteristics'=x,'OR'=OR,'CI'=CI,'P'=P)[-1,] 
...
pdf('forestplot_logistic.pdf', width = 8, height = 8, onefile = F)
...
core_expr <- exp2[consensus_genes, , drop = FALSE]

score <- as.numeric(ssgsea_scores["Ammonia_ICD", ])
names(score) <- colnames(exp2)

cor_list <- lapply(consensus_genes, function(g) {
...
write.csv(cor_df, "results/GSVA_core_gene_correlation.csv", row.names = FALSE)

# Scatter plots for UROD and PPIG
plot_cor <- function(gene) {
  df_plot <- data.frame(
    GSVA_Score = score,
    Expr = as.numeric(core_expr[gene, ]),
    Group = group_list
  )
  p <- ggplot(df_plot, aes(x = Expr, y = GSVA_Score)) +
    geom_point(aes(color = Group), size = 2.5, alpha = 0.8) +
    geom_smooth(method = "lm", se = TRUE, color = "black") +
    labs(
      x = paste(gene, "Expression"),
      y = "Ammonia-ICD ssGSEA Score",
      title = paste0(
        "Spearman R = ", round(cor_df$Rho[cor_df$Gene == gene], 3),
        ", p = ", signif(cor_df$Pval[cor_df$Gene == gene], 3)
      )
    ) +
    theme_classic()
  return(p)
...
pdf("figures/UROD_GSVA_correlation.pdf", width = 8, height = 6)
print(plot_cor("UROD"))
dev.off()

pdf("figures/PPIG_GSVA_correlation.pdf", width = 8, height = 6)
print(plot_cor("PPIG"))
dev.off()

# Heatmap of correlation matrix
cor_matrix <- cor(t(core_expr), score, method = "spearman", use = "complete.obs")
colnames(cor_matrix) <- "GSVA_score"

pdf("figures/GSVA_core_gene_heatmap.pdf", width = 6, height = 6)
pheatmap(
  cor_matrix,
  cluster_rows = TRUE,
  cluster_cols = FALSE,
  ...
  number_format = "%.3f",
  main = "Spearman correlation between core genes and ssGSEA score"
)
dev.off()
